...
import pymc3_ext.memoize
import pymc3_ext.util
from pymc3_ext.theanof import (
    change_flags,
    intX,
)
...
from pymc3_ext.variational.inference import (
    ADVI, FullRankADVI, SVGD, NFVI, ASVGD,
    fit
)
from pymc3_ext.variational import flows
from pymc3_ext.variational.opvi import Approximation, Group
from pymc3_ext.variational import opvi
from . import models
...
def three_var_aevb_groups(parametric_grouped_approxes, three_var_model, aevb_initial):
    dsize = np.prod(pymc3_ext.util.get_transformed(three_var_model.one).dshape[1:])
...
def test_clear_cache():
    import pickle
    pymc3_ext.memoize.clear_cache()
    assert all(len(c) == 0 for c in pymc3_ext.memoize.CACHE_REGISTRY)
    with pm.Model():
        pm.Normal('n', 0, 1)
        inference = ADVI()
        inference.fit(n=10)
        assert any(len(c) != 0 for c in inference.approx._cache.values())
        pymc3_ext.memoize.clear_cache(inference.approx)
        ...
        pymc3_ext.memoize.clear_cache(inference_new.approx)
