...
from pymc3_ext.step_methods.hmc import quadpotential
import pymc3_ext
...
def test_user_potential():
    ...
    with model:
        pymc3_ext.Normal("a", mu=0, sigma=1)
    ...
    with model:
        step = pymc3_ext.NUTS(potential=pot)
        pymc3_ext.sample(10, init=None, step=step, chains=1)
    assert called
...
def test_full_adapt_sampling(seed=289586):
    ...
    with pymc3_ext.Model() as model:
        pymc3_ext.MvNormal("a", mu=np.zeros(len(L)), chol=L, shape=len(L))

        pot = quadpotential.QuadPotentialFullAdapt(
            model.ndim, np.zeros(model.ndim)
        )
        step = pymc3_ext.NUTS(model=model, potential=pot)
        pymc3_ext.sample(
