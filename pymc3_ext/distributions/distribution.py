                ...
                if self.check_shape_in_random:
                    ...
                    if not expected_shape == samples.shape:
                        raise RuntimeError(
                            ...
                            "when the model uses pymc3_ext-ext.Data or theano shared "
...
def _draw_value(param, point=None, givens=None, size=None):
    ...
    Parameters
    ----------
    param : number, array like, theano variable or pymc3_ext-ext random variable
        The value or distribution. Constants or shared variables
        will be converted to an array and returned. Theano variables
        are evaluated. If `param` is a pymc3_ext-ext random variables, draw
        a new value from it and return that, unless a value is specified
        in `point`.
    point : dict, optional
        A dictionary from pymc3_ext-ext variable names to their values.
